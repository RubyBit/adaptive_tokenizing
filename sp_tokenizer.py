...
import sentencepiece_model_pb2 as proto_model
...
class SPTokenizer(PreTrainedTokenizerBase):
    ...
    def __init__(self, **kwargs):
        ...
        self.special_tokens_map_file = kwargs.pop("special_tokens_map_file",
                                                  os.path.dirname(self.name_or_path) + "/special_tokens_map.json")
    ...
    def tokenize(self, text, **kwargs):
        ...
        if 'tokenize_with_sampling' in kwargs:
            if kwargs['tokenize_with_sampling']:
                return self.sp.encode(text, nbest_size=-1, enable_sampling=kwargs['tokenize_with_sampling'], alpha=0.5,
                                      out_type=str)
            else:
    ...
    def save_vocabulary(self, save_directory: str, filename_prefix: Optional[str] = None):
        ...
        with open(os.path.split(name_or_path)[0] + "/" + os.path.split(self.name_or_path)[-1].split(".")[0] + ".vocab",
                  "w") as f:
            f.write(open(os.path.split(self.name_or_path)[0] + "/" + os.path.split(self.name_or_path)[-1].split(".")[
                0] + ".vocab", "r").read())
    ...
    def add_user_defined_tokens(self, added_tokens: List[str], type=4) -> None:
        ...
        with open(os.path.split(self.name_or_path)[0] + "/" + os.path.split(self.name_or_path)[-1].split(".")[
            0] + ".vocab", "w", encoding="utf-8") as f:
            for item in m.pieces:
                f.write(item.piece + "\t" + str(round(item.score, 5)) + "\n")
        self.sp = spm.SentencePieceProcessor(model_file=self.name_or_path)

    def build_mapping_file(self, mapping_tokenizer: PreTrainedTokenizerBase, mapping_file: str, max_length: int = 32,
                           is_chinese_vocab=False):
        mapping_index = []
        for idx in range(self.align_pos, 0):
            ...
            # mapping_ids.insert(0, self.sp.get_piece_size() + idx)
    ...
    def reindex_according_wordpiece(self, according_vocab_file, output_dir, control_tokens: Optional[Dict] = None,
                                    unknown_token: Optional[Dict] = None, according_mode: Optional[str] = "wordpiece",
                                    is_chinese_vocab=True, whitespace_placeholder=None, base_score=-50,
                                    prefix_subword="##"):
        '''
        ...
        '''
        ext = according_vocab_file.split(".")[-1]
        if ext == "txt":
            according_vocab = open(according_vocab_file, "r", encoding="uft-8").read().strip().split("\n")
            according_vocab = {i: according_vocab[i] for i in range(len(according_vocab))}
        elif ext == "json":
            according_vocab = json.load(open(according_vocab_file, encoding="utf-8"))
            according_vocab = {int(according_vocab[i]): i.replace(whitespace_placeholder,
                                                                  "▁") if whitespace_placeholder is not None else i for
                               i in according_vocab}
        elif ext == "vocab":
            according_vocab = open(according_vocab_file, "r").readlines()
            according_vocab = {i: according_vocab[i].split("\t")[0].strip() for i in range(len(according_vocab))}
            if according_mode == "sentencepiece":
                ...
                del temp_vocab
        elif ext == "model" and according_mode == "sentencepiece":
            m = proto_model.ModelProto()
            m.ParseFromString(open(according_vocab_file, "rb").read())
            pieces = m.pieces
            vocab = []
            according_vocab = {}
            ...
            for key, value in temp_vocab.items():
                if len(key) > 1 and "▁" == key[0]:
                    temp = key[1:]
                    while temp in temp_vocab:
                        temp = prefix_subword + temp
                    while temp != key[1:]:
                        according_vocab[temp_vocab[temp[2:]]] = temp
                        temp = temp[2:]
                    # if key[1:] in temp_vocab:
                    #     according_vocab[temp_vocab[key[1:]]] = "##" + key[1:]
                    according_vocab[value] = key[1:]
            del temp_vocab
        else:
            raise Exception("No available vocab file to load")

            with open(os.path.split(according_vocab_file)[0] + "/" + os.path.split(according_vocab_file)[-1].split(".")[
                0] + ".vocab", "w") as f:
                for item in vocab:
        ...
        for id, word in according_vocab.items():
            ...
            if new_token.type == 1:
                ...
                new_token.score = base_score * (len(new_token.piece) + 1) / len(
                    new_token.piece)  # theoretically this should be a value slightly higher than the biggest score shown among the incremental pieces
            else:
                new_token.piece = word
